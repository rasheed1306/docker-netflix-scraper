...
| File                 | What it does                                                                                                                                                                                                                                              |
| :------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `main.py`            | Orchestrates the full run — detects cold start vs. incremental, loops pages, calls everything                                                                                                                                                             |
| `db.py`              | All Supabase queries: load existing IDs, upsert movies, write ingestion logs                                                                                                                                                                              |
...
| youtube.py           | Calls `/search?part=snippet&type=video&maxResults=1&q={title} {year} official trailer&key={YOUTUBE_API_KEY}` — fallback only when TMDB `/videos` returns `NULL`; extracts `videoId` from response and returns `https://www.youtube.com/watch?v={videoId}` |
...
| `.env`               | Holds `TMDB_API_KEY`, `OMDB_API_KEY`, `OPENAI_API_KEY`, `SUPABASE_DB_URL`                                                                                                                                                                                 |
...
2. **Load existing IDs**: bulk fetch all `tmdb_id`s into a Python set — done once before the page loop.
3. **Per page (20 movies)**:
...
4. **Log the run** to `ingestion_logs` (`success` or `failed` + error message).
...
3. Check Supabase for 20 rows — verify ratings, embeddings, and some `NULL` trailer URLs (expected)
...
- New releases may have `NULL` ratings (OMDB not yet indexed) — handled gracefully
- `NULL` trailer URLs are expected — UI should conditionally hide the trailer button
- Laptop must stay on for the cold start; incremental runs are ~10–15 seconds[^2]


NOTE want to add:
- use tenacity for retry logic  make sure `db.py` also handles transient Postgres errors, not just the external APIs
